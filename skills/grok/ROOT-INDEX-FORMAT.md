...
The vault-root `INDEX.md` (`/Users/skhl/Vault/grok/INDEX.md`) is the registry of every topic — the browsable "what do I already know?" entry point for both the user and an agent reconstructing the user's knowledge.

It is a **derived projection** of every topic's frontmatter (ADR-0004): regenerated by rewriting it from all topic frontmatters on every confirmed checkpoint, and **never hand-edited**. Because it is a projection, every column must be a topic frontmatter field — nothing here is authored directly, so it cannot drift.
